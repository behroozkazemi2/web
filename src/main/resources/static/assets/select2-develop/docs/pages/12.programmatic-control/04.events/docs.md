...
You can manually trigger events on a Select2 control using the jQuery [`trigger`](https://api.jquery.com/trigger/) method.  However, if you want to pass some data to any handlers for the event, you need to construct a new [jQuery `Event` object](https://api.jquery.com/category/events/event-object/) and trigger on that:
